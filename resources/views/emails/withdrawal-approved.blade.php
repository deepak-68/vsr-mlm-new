<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Withdrawal Update</title></head>
...
<h1 style="color: #fff; margin: 0; font-size: 22px;">Withdrawal {{ ucfirst($status) }}</h1>
</div>
<div style="padding: 30px;">
<p>Dear <strong>{{ $user->first_name }} {{ $user->last_name }}</strong>,</p>
<p>Your withdrawal request of <strong>₹{{ number_format($amount, 2) }}</strong> has been <strong>{{ $status }}</strong>.</p>
@if($status === 'approved')
<p>The amount will be credited to your registered bank account shortly.</p>
